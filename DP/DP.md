...
> ##### 题目描述
>
> 给定一个由 $n$ 行构成的数字三角形。第 $i$ 行共有 $2i-1$ 个整数。
>
> 从顶点（第一行唯一的数字）出发，依次向下移动恰好 $n-1$ 次直到抵达最后一行。
>
> 假设当前位于第 $i$ 行第 $j$ 列：
> 1. 可以向**正下方**移动至第 $(i+1)$ 行第 $j$ 列；
> 2. 可以向**左下方**移动至第 $(i+1)$ 行第 $(j-1)$ 列；
> 3. 可以向**右下方**移动至第 $(i+1)$ 行第 $(j+1)$ 列。
>
...
> ##### 输入描述
>
> 第一行输入两个整数 $n, k$ ($1 \le n \le 300; 0 \le k \le n$)，分别表示数字三角形的行数与允许的移动差。
>
> 此后 $n$ 行，第 $i$ 行输入 $2i-1$ 个整数：
> $a_{i,1}, a_{i,2}, \dots, a_{i, 2i-1}$
>
> 数据范围：
> $-2 \times 10^9 \le a_{i,j} \le 2 \times 10^9$
>
...
所谓 $k$ ,其实就是代表与中间的距离. 这里需要定义二维数组 $dp[i][j]$ ,

所以我们首先先用一个二维数组把三角形存起来, 同时用一个非常小的数字标记不可达点

```cpp
cin>>n>>k;
int square=2*n-1;

memset(maap,-INF,sizeof(maap));
memset(dp,-INF,sizeof(dp));

for(int i=0;i<n;i++)
{
    int l=square/2+1-i; //起始点
    int r=2*(i+1)-1;
    for(int j=l;j<=r;j++)
    {
        cin>>maap[i+1][j];
...
由于必须移动到最后一行, 所以最大值肯定在最后一行取, 所以此时使用k进行筛选
